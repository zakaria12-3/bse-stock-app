...
readonly class SaleData
{
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            build_name: !empty($data['build_name']) ? $data['build_name'] : null,
            customer_id: isset($data['customer_id']) ? (int) $data['customer_id'] : null,
            status: isset($data['status']) ? SaleStatus::from($data['status']) : SaleStatus::COMPLETED,
            notes: $data['notes'] ?? null,
            cash_received: round((float) ($data['cash_received'] ?? 0), 3),
            change: round((float) ($data['change'] ?? 0), 3),
            global_discount: round((float) ($data['global_discount'] ?? 0), 3),
        );
    }
