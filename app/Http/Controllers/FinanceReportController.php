...
class FinanceReportController extends Controller
{
    public function print(Request $request, string $printId)
    {
        ...
        if (!$selectedIds) {
            abort(404, 'La session d impression a expire ou est invalide. Selectionnez les transactions puis reessayez.');
        }
        ...
        if ($cashFlows->isEmpty()) {
             abort(404, 'Aucune transaction trouvee');
        }
        ...
        $storeName = Setting::get('store_name', 'BSE Industrie');
        $storeAddress = Setting::get('store_address', 'Adresse BSE');
        ...
        if ($periodKey) {
            ...
            switch ($periodKey) {
                case 'today':
                    $periodText = 'Aujourd hui (' . $now->translatedFormat('d F Y') . ')';
                    break;
                case 'yesterday':
                    $periodText = 'Hier (' . $now->subDay()->translatedFormat('d F Y') . ')';
                    break;
                case 'this_week':
                    $periodText = 'Cette semaine (' . $now->startOfWeek()->translatedFormat('d M') . ' - ' . $now->endOfWeek()->translatedFormat('d M Y') . ')';
                    break;
                case 'last_week':
                    $periodText = 'Semaine derniere (' . $now->subWeek()->startOfWeek()->translatedFormat('d M') . ' - ' . $now->subWeek()->endOfWeek()->translatedFormat('d M Y') . ')';
                    break;
                case 'this_month':
                    $periodText = 'Ce mois (' . $now->translatedFormat('F Y') . ')';
                    break;
                case 'last_month':
                    $periodText = 'Mois dernier (' . $now->subMonth()->translatedFormat('F Y') . ')';
                    break;
            }
        }
