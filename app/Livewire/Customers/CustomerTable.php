...
final class CustomerTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Inscrit le', 'created_at_formatted', 'created_at')
                ->sortable()
        ...
        return [
            ...
            Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "Voulez-vous vraiment supprimer le client '{$row->name}' ? Cette action est definitive.",
                ])
    ...
    public function delete($rowId, CustomerService $customerService): void
    {
        ...
        if ($customer) {
            try {
                ...
                $this->dispatch('toast', message: 'Client supprime avec succes.', type: 'success');
            } catch (\Exception $e) {
                // Ensure we catch our custom exception or fallback to general exception
                $message = $e instanceof CustomerException
                    ? $e->getMessage()
                    : 'Echec de la suppression du client : ' . $e->getMessage();
