...
final class FinanceTransactionTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('reference_export', function (FinanceTransaction $model) {
                $tag = $model->reference_type ? 'Auto' : 'Manual';
                if (!empty($model->external_reference)) {
                    return $tag . ' ' . $model->external_reference;
                }
                return $tag . ' ' . $model->code;
            ...
            ->add('type_export', fn(FinanceTransaction $model) => $model->category->type->value)
            ...
            ->add('amount_export', function (FinanceTransaction $model) {
                $type = $model->category->type->value ?? '';
                $prefix = $type === 'income' ? '+' : '-';
                return $prefix . ' ' . format_money($model->amount);
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Reference', 'reference_export', 'code')
                ->hidden()
                ->visibleInExport(true),
            ...
            Column::make('Montant', 'amount_formatted', 'amount')
                ->sortable()
                ->headerAttribute('text-right')
                ->bodyAttribute('text-right')
                ->visibleInExport(false),

            Column::make('Montant', 'amount_export', 'amount')
                ->hidden()
                ->visibleInExport(true),
    ...
    public function actions(FinanceTransaction $row): array
    {
        ...
        if (is_null($row->reference_type)) {
            $actions[] = Button::add('edit')
                ...
                ->tooltip('Modifier la transaction');
            ...
            $actions[] = Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "Voulez-vous vraiment supprimer cette transaction ? Cette action est definitive.",
                ])
    ...
    public function bulkPrint(): void
    {
        ...
        if (empty($checkboxValues)) {
            $this->dispatch('toast', message: 'Aucune transaction selectionnee.', type: 'warning');
            return;
        }
    ...
    public function delete($rowId, FinanceTransactionService $service): void
    {
        ...
        if ($transaction) {
            try {
                ...
                $this->dispatch('toast', message: 'Transaction supprimee avec succes.', type: 'success');
            } catch (\Exception $e) {
                $message = $e instanceof FinanceTransactionException
                    ? $e->getMessage()
                    : 'Echec de la suppression de la transaction : ' . $e->getMessage();
