...
final class SalesTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('build_name', fn(Sale $model) => $model->build_name ?: '-')
            ->add('customer_name', fn(Sale $model) => $model->customer ? $model->customer->name : 'Client passager')
            ...
            ->add('status_export', fn(Sale $model) => $model->status->label())
            ->add('date_period', fn() => '')
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Facture', 'invoice_number')
                ->searchable()
                ->sortable(),

            Column::make('Amenagement', 'build_name')
                ->searchable()
                ->sortable(),

            Column::make('Client', 'customer_name', 'customer_id')
                ->searchable()
                ->sortable(),
        ...
        return [
            ...
            Filter::select('date_period')
                ...
                ->builder(function (Builder $query, string $value) {
                    switch ($value) {
                        case 'today':
                            $query->whereBetween('sale_date', [now()->startOfDay(), now()->endOfDay()]);
                            break;
                        case 'yesterday':
                            $yesterday = now()->subDay();
                            $query->whereBetween('sale_date', [$yesterday->copy()->startOfDay(), $yesterday->copy()->endOfDay()]);
                            break;
                        case 'this_week':
                        ...
                        case 'last_month':
                            ...
                            $query->whereBetween('sale_date', [$lastMonth->copy()->startOfMonth(), $lastMonth->copy()->endOfMonth()]);
                            break;
                    }
                }),
        ];
        ...
        return [
            ...
            Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "Voulez-vous vraiment supprimer definitivement la facture '{$row->invoice_number}' ? Cette action est definitive.",
                ])
    ...
    public function delete($rowId, SaleService $saleService): void
    {
        ...
        if ($sale) {
            try {
                ...
                $this->dispatch('toast', message: 'Vente supprimee avec succes.', type: 'success');
            } catch (SaleException $e) {
                $this->dispatch('toast', message: 'Echec de la suppression : ' . $e->getMessage(), type: 'error');
            } catch (\Exception $e) {
