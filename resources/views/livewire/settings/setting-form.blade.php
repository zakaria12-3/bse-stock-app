    ...
    <div class="p-6">
        ...
        <div class="mb-6 space-y-1.5 text-center sm:text-left border-b border-gray-200 pb-4">
            <h3 class="text-lg font-semibold leading-none tracking-tight text-foreground">
                {{ __('Modifier le parametre') }}
            </h3>
            <p class="text-sm text-muted-foreground">
                {{ __('Modifiez la valeur de ce parametre.') }}
            </p>
        </div>
        ...
        <form wire:submit="save" class="space-y-4">
            <!-- Label (Readonly) -->
            <div class="space-y-2">
                <x-input-label for="key" :value="__('Nom du parametre')" />
            ...
            <div class="space-y-2">
                ...
                @if($key === 'currency_position')
                    <select id="value" wire:model="value" class="block w-full rounded-md border-input bg-background shadow-sm focus:border-ring focus:ring-ring sm:text-sm">
                        <option value="left">Gauche (exemple : DT 10.000)</option>
                        <option value="right">Droite (exemple : 10.000 DT)</option>
                    </select>
                @elseif($key === 'currency_fraction_digits')
                    <input 
                        ...
                        placeholder="0 pour DT sans decimales, 2 avec decimales"
                    >
