<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
        <!-- Fournisseur -->
        <div class="space-y-2">
            <x-input-label for="supplier_id" :value="__('Fournisseur')" required />
            <div class="w-full">
                <select id="supplier_id" name="supplier_id"
                        x-init="initFournisseurSelect($el)"
                        x-model="supplier_id"
                        autocomplete="off">
                    <option value=""></option>
                    @if(old('supplier_id'))
        ...
        <!-- Invoice (Optional) -->
        <div class="space-y-2">
            <x-input-label for="invoice_number" :value="__('Numero de facture (facultatif)')" />
            <x-text-input
                id="invoice_number"
                type="text"
                name="invoice_number"
                :value="old('invoice_number', $purchase->invoice_number ?? '')"
                placeholder="Laisser vide pour les brouillons"
                class="block w-full"
            />
        ...
        <!-- Proof Image -->
        <div class="space-y-2">
            <x-input-label for="proof_image" :value="__('Justificatif de reception')" />
            <input
                id="proof_image"
                type="file"
                name="proof_image"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
                <x-input-label for="purchase_date" :value="__('Date d achat')" required />
                <x-text-input
                    id="purchase_date"
                    type="date"
                    name="purchase_date"
            ...
            <div class="space-y-2">
                <x-input-label for="due_date" :value="__('Date d echeance')" />
                <x-text-input
                    id="due_date"
                    type="date"
                    name="due_date"
         ...
         <div class="space-y-2">
            ...
            <div class="flex h-10 w-full items-center rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-500">
                {{ isset($purchase) && $purchase->status ? $purchase->status->label() : 'Brouillon (par defaut)' }}
            </div>
        </div>
    ...
    <div class="space-y-4">
        ...
        <div class="relative z-20">
             <select
                ...
                placeholder="Rechercher une piece a ajouter..."
                autocomplete="off"
            ></select>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 sticky top-0 z-10">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Nouveau PR HT dossier</th>
                            <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Sous-total</th>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-20 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-base font-medium">Aucun article ajoute</p>
                                        <p class="text-sm text-gray-400">Recherchez des pieces ci-dessus pour les ajouter a l'achat</p>
                                    </div>
                                </td>
                            </tr>
                        </template>
                    ...
                    <tfoot class="bg-gray-50 border-t border-gray-200">
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-right font-bold text-gray-900 text-base">Total achat :</td>
    ...
    <div class="flex items-center justify-end gap-x-4 pt-6 border-t border-gray-200">
        ...
        <x-primary-button class="flex items-center gap-2" ::disabled="loading">
            ...
            <span x-text="loading ? 'Traitement...' : ({{ isset($purchase->id) ? '`Mettre a jour l achat`' : '`Creer un achat`' }})"></span>
        </x-primary-button>
    </div>
</div>
            ...
            addProduct(product) {
                ...
                if (existingIndex !== -1) {
                    ...
                    window.dispatchEvent(new CustomEvent('toast', {
                        detail: {
                            message: 'Piece deja presente. Quantite mise a jour.',
                            type: 'info'
                        }
                    }));
                } else {
                    ...
                    window.dispatchEvent(new CustomEvent('toast', {
                        detail: {
                            message: 'Piece "' + product.text + '" ajoutee a la liste.',
                            type: 'success'
                        }
                    }));
                }
