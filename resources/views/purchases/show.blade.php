<x-app-layout title="Details de l'achat">
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-foreground leading-tight">
                {{ __('Details de l achat') }} #{{ $purchase->invoice_number ?: $purchase->id }}
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden border border-gray-200">
                <div class="p-6">
                    ...
                    <div class="flex items-start justify-between border-b border-gray-100 pb-4 mb-6">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900">{{ __('Informations de l achat') }}</h3>
                            <p class="text-sm text-gray-500">{{ __('Details de la transaction d achat') }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <!-- Supplier -->
                        <x-detail-item label="Fournisseur" :value="$purchase->supplier->name">
                            <x-heroicon-o-building-storefront class="w-4 h-4 text-gray-400" />
                        </x-detail-item>

                        <!-- Invoice -->
                        <x-detail-item label="Numero de facture" :value="$purchase->invoice_number ?? '-'">
                            <x-heroicon-o-document-text class="w-4 h-4 text-gray-400" />
                        </x-detail-item>

                        <!-- Purchase Date -->
                        <x-detail-item label="Date d'achat" :value="$purchase->purchase_date->format('d M Y')">
                            <x-heroicon-o-calendar class="w-4 h-4 text-gray-400" />
                        </x-detail-item>

                        <!-- Due Date -->
                        <x-detail-item label="Date d'echeance" :value="$purchase->due_date ? $purchase->due_date->format('d M Y') : '-'">
                            <x-heroicon-o-calendar class="w-4 h-4 text-gray-400" />
                        </x-detail-item>
                        ...
                        <!-- Total Amount -->
                        <x-detail-item label="Montant total" :value="format_money($purchase->total)">
                            <x-heroicon-o-cube class="w-4 h-4 text-gray-400" />
                        </x-detail-item>

                        <!-- Created By -->
                        <x-detail-item label="Cree par" :value="$purchase->creator->name ?? 'Inconnu'">
                        ...
                        @if($purchase->proof_image)
                            <div>
                                <label class="text-sm font-medium leading-none text-gray-500">Justificatif de reception</label>
                                <div class="mt-1">
                        ...
                        @else
                            <x-detail-item label="Justificatif de reception" value="-" />
                        @endif
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <div class="space-y-1">
                            ...
                            <div class="bg-gray-50 p-3 rounded-md border border-gray-100">
                                <p class="text-sm text-slate-700 italic leading-relaxed">{{ $purchase->notes ?: 'Aucune note supplementaire.' }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-right">Prix d'achat</th>
                                    <th class="px-6 py-3 text-right">Nouveau PR HT dossier</th>
            ...
            <div x-data="{
                ...
                @if($purchase->status === \App\Enums\PurchaseStatus::DRAFT)
                    ...
                    <x-danger-button
                        type="button"
                        @click="confirmAction('{{ route('purchases.destroy', $purchase) }}', 'DELETE', 'Supprimer le brouillon', 'Voulez-vous vraiment supprimer ce brouillon ? Cette action est definitive.', 'Supprimer le brouillon', '!bg-red-600 hover:!bg-red-700 focus:!ring-red-500')"
                    >
                        {{ __('Supprimer le brouillon') }}
                    </x-danger-button>
                    ...
                    <x-primary-button
                        ...
                        @click="confirmAction('{{ route('purchases.mark-ordered', $purchase) }}', 'PATCH', 'Marquer comme commande', 'Voulez-vous marquer cet achat comme commande ? Le stock ne sera mis a jour qu a la reception des articles.', 'Marquer comme commande', '!bg-sky-600 hover:!bg-sky-700 focus:!ring-sky-500')"
                    >
                        {{ __('Marquer comme commande') }}
                    </x-primary-button>
                ...
                @elseif($purchase->status === \App\Enums\PurchaseStatus::ORDERED)
                    ...
                    <x-secondary-button
                        ...
                        @click="confirmAction('{{ route('purchases.cancel', $purchase) }}', 'PATCH', 'Annuler la commande', 'Voulez-vous vraiment annuler cette commande ?', 'Annuler la commande', '!bg-red-600 hover:!bg-red-700 focus:!ring-red-500')"
                    >
                        ...
                        <div x-show="open"
                            ...
                            <div @click.outside="open = false"
                                ...
                                <h3 class="text-lg font-medium text-gray-900 mb-4">
                                    Reception de l'achat #{{ $purchase->invoice_number ?? $purchase->id }}
                                </h3>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        @if($purchase->invoice_number)
                                            <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                                                <span class="block text-xs font-medium text-gray-500 uppercase">Numero de facture</span>
                                                <span class="text-sm font-semibold text-gray-900">{{ $purchase->invoice_number }}</span>
                                            </div>
                                        @else
                                            <div class="space-y-2">
                                                <x-input-label for="invoice_number" :value="__('Numero de facture final')" required />
                                                <x-text-input
                                                    id="invoice_number"
                                                    name="invoice_number"
                                        ...
                                        @if($purchase->proof_image)
                                            <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                                                <span class="block text-xs font-medium text-gray-500 uppercase mb-1">Justificatif de reception</span>
                                                <a href="{{ Storage::url($purchase->proof_image) }}" target="_blank" class="text-indigo-600 hover:underline text-sm flex items-center gap-1">
                                                    <x-heroicon-o-paper-clip class="w-4 h-4" />
                                                    Voir l'image envoyee
                                                </a>
                                            </div>
                                        @else
                                            <div class="space-y-2">
                                                <x-input-label for="proof_image" :value="__('Ajouter le justificatif de reception')" required />
                                                <input
                                                    id="proof_image"
                                                    type="file"
                                                    name="proof_image"
                                                ...
                                                />
                                                <p class="text-xs text-gray-500">Image (JPG, PNG) max 2 Mo.</p>
                                        ...
                                        @if($purchase->invoice_number && $purchase->proof_image)
                                            <p class="text-xs text-green-600 mt-3 font-medium flex items-center">
                                                ...
                                                Donnees completes. Pret pour la reception.
                                            </p>
                                        @endif
                                    </div>
                ...
                @elseif($purchase->status === \App\Enums\PurchaseStatus::RECEIVED)
                    ...
                    <x-primary-button
                        ...
                        @click="confirmAction('{{ route('purchases.mark-paid', $purchase) }}', 'PATCH', 'Marquer comme paye', 'Voulez-vous marquer cet achat comme paye ? Cela suppose que le montant total a ete regle.', 'Marquer comme paye', '!bg-emerald-600 hover:!bg-emerald-700 focus:!ring-emerald-500')"
                    >
                        <x-heroicon-o-document-check class="w-5 h-5 mr-1" />
                        {{ __('Marquer comme paye') }}
                    </x-primary-button>
                ...
                @elseif($purchase->status === \App\Enums\PurchaseStatus::CANCELLED)
                    ...
                    <x-secondary-button
                        type="button"
                        @click="confirmAction('{{ route('purchases.restore-draft', $purchase) }}', 'PATCH', 'Restaurer en brouillon', 'Remettre cet achat en brouillon ? Vous pourrez le modifier a nouveau.', 'Restaurer en brouillon', '!bg-gray-800 hover:!bg-gray-700 text-white')"
                    >
                        {{ __('Restaurer en brouillon') }}
                    </x-secondary-button>
