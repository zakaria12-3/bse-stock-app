<x-app-layout title="Dossier ambulance">
    <div class="mx-auto sm:px-6 lg:px-8 py-4"
         ...
         @keydown.window.f4.prevent="openClientModal()"
    >
        <div class="flex flex-col lg:flex-row h-[calc(100vh-100px)] space-y-4 lg:space-y-0 lg:space-x-4 relative">
            ...
            <div class="w-full lg:w-[70%] flex flex-col space-y-4 h-full">
                ...
                <div class="relative z-20 mb-2">
                    <select
                        x-ref="productSelect"
                        placeholder="Rechercher une piece (designation ou reference) [F1]..."
                        autocomplete="off"
                    ></select>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">CUMP / PR HT</th>
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Qte</th>
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Unite</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Pose/unite</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Prix installe</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Remise/unite</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <template x-for="(item, index) in cart" :key="item.id">
                                    <tr :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-50'" class="hover:bg-indigo-50 transition-colors">
                                            ...
                                            <div class="space-y-2">
                                                <div class="flex items-center justify-end gap-2">
                                                    <input
                                                        ...
                                                        min="0"
                                                        step="0.25"
                                                        x-model.number="item.labor_hours"
                                                        class="w-20 text-right border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-sm shadow-sm"
                                                        placeholder="h"
                                                        title="Heures de pose par unite"
                                                    >
                                                    <span class="text-xs text-gray-400">h</span>
                                                </div>
                                                <div class="relative rounded-md shadow-sm w-28 ml-auto">
                                                    ...
                                                    <input
                                                        type="text"
                                                        :value="formatNumber(item.labor_rate)"
                                                        @input="item.labor_rate = unformatNumber($event.target.value)"
                                                ...
                                                <div class="text-[11px] text-gray-500">
                                                    MO: <span x-text="formatCurrency(lineLabor(item))"></span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-700" x-text="formatCurrency(lineInstalledUnitPrice(item))"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold text-gray-900" x-text="formatCurrency(lineTotal(item))"></td>
                                    ...
                                    <tr>
                                        <td colspan="9" class="px-6 py-20 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="text-base font-medium">Aucune piece ajoutee</p>
                                                <p class="text-sm text-gray-400">Recherchez les composants du dossier ci-dessus</p>
                                            </div>
                                        </td>
                                    </tr>
                                </template>
            ...
            <div class="w-full lg:w-[30%] flex flex-col bg-white rounded-lg shadow border border-gray-200 h-full">
                ...
                <div class="p-4 border-b border-gray-200 bg-gray-50 rounded-t-lg">
                    <h2 class="text-xs font-bold text-gray-500 uppercase tracking-wide">Details du dossier</h2>
                </div>

                <div class="p-4 space-y-6 flex-1 overflow-y-auto">
                    <div class="space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                        <label class="block text-xs font-bold text-gray-500 uppercase">Nom du dossier ambulance</label>
                        <input
                            type="text"
                            x-model="payment.build_name"
                            class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="Ex. Ambulance categorie B - Unite 01"
                        >
                        <p class="text-xs text-gray-500">
                            Nommez le vehicule, l'amenagement, la renovation ou le dossier SAV.
                        </p>
                    </div>
                    ...
                    <div class="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <label class="block text-xs font-bold text-blue-700 uppercase">Reutiliser un dossier</label>
                                <p class="mt-1 text-xs text-blue-600">
                                    Reprenez une configuration existante, puis ajustez les composants selon le vehicule.
                                </p>
                            </div>
                            <button
                                type="button"
                                @click="clearLoadedBuild()"
                                x-show="loadedBuild"
                                class="rounded-md border border-blue-200 bg-white px-2.5 py-1 text-xs font-semibold text-blue-700 hover:bg-blue-100"
                            >
                                Vider
                            </button>
                        </div>

                        <div x-show="!loadedBuild">
                            <select
                                x-ref="buildSelect"
                                placeholder="Rechercher par nom de dossier ou facture..."
                                autocomplete="off"
                            ></select>
                        </div>

                        <template x-if="loadedBuild">
                            <div class="rounded-md border border-blue-200 bg-white px-3 py-3 text-sm">
                                <div class="font-semibold text-gray-900" x-text="loadedBuild.build_name"></div>
                                <div class="mt-1 text-xs text-gray-500">
                                    <span x-text="loadedBuild.invoice_number"></span>
                                    <span x-show="loadedBuild.sale_date">&bull;</span>
                                    <span x-text="loadedBuild.sale_date"></span>
                                </div>
                                <div class="mt-1 text-xs text-gray-500" x-show="loadedBuild.customer_name">
                                    Client: <span x-text="loadedBuild.customer_name"></span>
                                </div>
                                <div class="mt-2 text-xs font-medium text-blue-700">
                                    Configuration chargee. Vous pouvez modifier les quantites et ajouter des pieces.
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <div class="bg-indigo-50 rounded-lg p-4 border border-indigo-100 relative group">
                        <div class="flex justify-between items-start mb-2">
                            <span class="text-xs font-bold text-indigo-500 uppercase">Client</span>
                            <button @click="openClientModal()" class="text-[10px] font-semibold text-indigo-600 hover:text-white hover:bg-indigo-600 border border-indigo-200 bg-white px-2 py-1 rounded transition-colors flex items-center">
                                + Nouveau (F4)
                            </button>
                        </div>
                        ...
                        <div class="relative">
                            <template x-if="selectedClient">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <h3 class="font-bold text-lg text-gray-900" x-text="selectedClient.name"></h3>
                                        <p class="text-sm text-gray-600" x-text="selectedClient.phone || 'Aucun telephone'"></p>
                                    </div>
                                    <button @click="resetClient()" class="text-gray-400 hover:text-red-500">
                            ...
                            <div x-show="!selectedClient">
                                <select
                                    ...
                                    placeholder="Rechercher un client [F2]..."
                                    autocomplete="off"
                                ></select>
                            </div>
                        </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-slate-500 text-sm" x-show="laborSubtotal > 0">
                            <span>Main d'oeuvre</span>
                            <span x-text="formatCurrency(laborSubtotal)"></span>
                        </div>
                        <div class="flex justify-between items-center mt-2">
                             <span class="text-sm font-medium text-gray-500">Remise globale</span>
                    ...
                    <div class="space-y-4 pt-4 border-t border-gray-200">
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Mode de paiement</label>
                        ...
                        <template x-if="payment.method === 'cash'">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Montant recu</label>
                                <div class="relative">
                                ...
                                <div class="bg-green-50 p-3 rounded-md border border-green-100 flex justify-between items-center mt-2"
                                    ...
                                    <span class="text-sm font-medium uppercase" x-text="change < 0 ? 'Reste' : 'Monnaie'"></span>
                        ...
                        <div>
                            <textarea
                                ...
                                placeholder="Notes du dossier, vehicule, categorie A/B/4x4, EN1789, BSEBact, SAV..."
                            ></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 border-t border-gray-200 bg-gray-50 flex gap-3">
                    ...
                    >
                        ...
                        <span x-text="isSubmitting ? 'Traitement...' : 'VALIDER (F3)'"></span>
                    </button>
                </div>
            </div>
        </div>
            ...
            function pos() {
                return {
                    ...
                    init() {
                        ...
                        if (savedCart) this.cart = JSON.parse(savedCart).map(item => this.normalizeCartItem(item));

                        const savedClient = localStorage.getItem('pos_customer');
                        if (savedClient) this.selectedClient = JSON.parse(savedClient);

                        const savedLoadedBuild = localStorage.getItem('pos_loadedBuild');
                        if (savedLoadedBuild) this.loadedBuild = JSON.parse(savedLoadedBuild);
                        ...
                        this.$watch('selectedClient', (val) => localStorage.setItem('pos_customer', JSON.stringify(val)));
                        this.$watch('loadedBuild', (val) => localStorage.setItem('pos_loadedBuild', JSON.stringify(val)));
                        ...
                        this.productTs = new TomSelect(this.$refs.productSelect, {
                            ...
                            load: (query, callback) => {
                                ...
                                fetch('{{ route("ajax.products.search") }}', {
                                    ...
                                    body: JSON.stringify({ q: query, in_stock_only: true })
                                })
                                        ...
                                        <div class="py-2 px-3 border-b border-gray-100">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <div class="text-right">
                                                    <div class="font-bold text-indigo-600">${this.formatCurrency(item.unit_pr_ht ?? item.selling_price)}</div>
                        ...
                        this.customerTs = new TomSelect(this.$refs.customerSelect, {
                                ...
                                option: (item, escape) => {
                                    return `
                                        <div class="py-2 px-3 hover:bg-indigo-50">
                                            <div class="font-medium text-gray-900">${escape(item.name)}</div>
                                            <div class="text-xs text-gray-500">${escape(item.phone || 'Aucun telephone')}</div>
                                        </div>
                                    `;
                                }
                            },
                            onChange: (value) => {
                                if (value) {
                                    ...
                                    if(item) {
                                        // Keep the client object consistent with the selected card.
                                        this.selectedClient = {
                                            id: item.value,
                                            name: item.name,
                                            phone: item.phone,
                    ...
                    initBuildSelect() {
                        if (!this.$refs.buildSelect) return;

                        if (this.buildTs) {
                            this.buildTs.destroy();
                            this.buildTs = null;
                        }

                        this.buildTs = new TomSelect(this.$refs.buildSelect, {
                            valueField: 'value',
                            labelField: 'name',
                            searchField: ['name', 'invoice_number'],
                            preload: 'focus',
                            openOnFocus: true,
                            load: (query, callback) => {
                                fetch('{{ route("ajax.sales-builds.search") }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                                ...
                                option: (item, escape) => {
                                    return `
                                        <div class="py-2 px-3 border-b border-gray-100">
                                            <div class="font-medium text-gray-900">${escape(item.name)}</div>
                                            <div class="mt-1 text-xs text-gray-500">
                                                ${escape(item.invoice_number || '')}${item.sale_date ? ' &bull; ' + escape(item.sale_date) : ''}
                                            </div>
                                            ${item.customer_name ? `<div class="text-xs text-gray-500">Client: ${escape(item.customer_name)}</div>` : ''}
                                        </div>
                                    `;
                                },
                            ...
                            onChange: async (value) => {
                                ...
                                await this.loadPreviousBuild(value);

                                this.buildTs.clear(true);
                                this.buildTs.setTextboxValue('');
                            }
                        });
                    },
                    ...
                    async loadPreviousBuild(buildId) {
                        if (this.cart.length > 0) {
                            const shouldReplace = window.confirm('Le chargement d un dossier precedent remplacera le panier actuel. Continuer ?');
                            if (!shouldReplace) {
                                return;
                            }
                        }
                        ...
                        try {
                            const response = await fetch(`{{ url('/ajax/sales-builds') }}/${buildId}`, {
                                headers: {
                            ...
                            if (!response.ok) {
                                this.$dispatch('toast', { message: data.message || 'Impossible de charger ce dossier.', type: 'error' });
                                return;
                            }
                            ...
                            data.items.forEach((entry) => {
                                ...
                                if (!product) {
                                    warnings.push(`Une piece du dossier ${data.build_name} n existe plus et a ete ignoree.`);
                                    return;
                                }

                                const maxStock = parseInt(product.quantity || 0);
                                if (maxStock <= 0) {
                                    warnings.push(`${product.name} est en rupture de stock et a ete ignoree.`);
                                    return;
                                }

                                const requestedQty = parseInt(entry.template_quantity || 1);
                                const quantity = Math.min(requestedQty, maxStock);

                                if (quantity < requestedQty) {
                                    warnings.push(`${product.name} a ete reduite a ${quantity}, car il ne reste que ${maxStock} en stock.`);
                                }

                                loadedItems.push(this.normalizeCartItem({
                                    id: product.id,
                                    name: product.name,
                                    sku: product.sku,
                                    price: product.unit_pr_ht ?? product.price,
                                    quantity: quantity,
                                    max_stock: maxStock,
                                    unit: product.unit ? product.unit.symbol : '',
                                    discount: parseFloat(entry.template_discount || 0),
                                    labor_hours: parseFloat(entry.template_labor_hours || 0),
                                    labor_rate: parseFloat(entry.template_labor_rate || 0)
                                }));
                            });

                            if (loadedItems.length === 0) {
                                this.$dispatch('toast', { message: 'Aucune piece reutilisable en stock n a ete trouvee dans ce dossier.', type: 'error' });
                                return;
                            }

                            this.cart = loadedItems;
                            this.loadedBuild = {
                                id: data.id,
                            ...
                            this.payment.build_name = data.build_name || this.payment.build_name;
                            ...
                            this.$dispatch('toast', { message: `Loaded ${loadedItems.length} pieces from ${data.build_name}.`, type: 'success' });
                            ...
                            console.error(error);
                            this.$dispatch('toast', { message: 'Erreur reseau pendant le chargement du dossier.', type: 'error' });
                        }
                    },
                    ...
                    clearStorage() {
                        ...
                        localStorage.removeItem('pos_loadedBuild');
                    ...
                    addToCart(product) {
                        ...
                        if (existing) {
                            if (existing.quantity < product.quantity) {
                                existing.quantity++;
                                this.$dispatch('toast', { message: 'Piece deja presente. Quantite mise a jour.', type: 'info' });
                            } else {
                                this.$dispatch('toast', { message: 'Stock insuffisant.', type: 'error' });
                            }
                        } else {
                            if (product.quantity > 0) {
                                this.cart.push(this.normalizeCartItem({
                                    id: product.id,
                                    name: product.name,
                                    sku: product.sku,
                                    price: product.unit_pr_ht ?? product.selling_price,
                                    quantity: 1,
                                    max_stock: product.quantity,
                                    unit: product.unit ? product.unit.symbol : '',
                                    discount: 0
                                }));
                                this.$dispatch('toast', { message: 'Piece "' + product.name + '" ajoutee au dossier.', type: 'success' });
                            } else {
                                this.$dispatch('toast', { message: 'Rupture de stock.', type: 'error' });
                            }
                        }
                    },
                    ...
                    validateQty(index) {
                        ...
                        if (item.quantity > item.max_stock) {
                            ...
                            this.$dispatch('toast', { message: 'Stock maximum atteint.', type: 'warning' });
                        }
                        if (item.quantity < 1) item.quantity = 1;
                    },

                    lineLabor(item) {
                        return (parseFloat(item.labor_hours) || 0) * (parseFloat(item.labor_rate) || 0);
                    },

                    lineInstalledUnitPrice(item) {
                        return (parseFloat(item.price) || 0) + this.lineLabor(item);
                    },

                    lineTotal(item) {
                        const discount = parseFloat(item.discount) || 0;
                        const quantity = parseInt(item.quantity) || 0;
                        return Math.max(this.lineInstalledUnitPrice(item) - discount, 0) * quantity;
                    },

                    removeFromCart(index) {
                        const removedItem = this.cart[index];
                        this.cart.splice(index, 1);
                        this.$dispatch('toast', { message: 'Piece "' + removedItem.name + '" retiree du dossier.', type: 'info' });
                    },
                    ...
                    get subtotal() {
                        return this.cart.reduce((sum, item) => sum + (this.lineInstalledUnitPrice(item) * item.quantity), 0);
                    },

                    get laborSubtotal() {
                        return this.cart.reduce((sum, item) => sum + (this.lineLabor(item) * (parseInt(item.quantity) || 0)), 0);
                    },
                    ...
                    openConfirmation() {
                        ...
                        if (this.payment.method === 'cash' && this.payment.cash_received < this.total) {
                            this.$dispatch('toast', { message: 'Paiement insuffisant.', type: 'error' });
                            return;
                        }
                    ...
                    async submitSale() {
                        ...
                        try {
                            ...
                            const payload = {
                                ...
                                build_name: this.payment.build_name,
                                items: items,
                            ...
                            if (res.ok && data.success) {
                                ...
                                this.$dispatch('toast', { message: 'Dossier valide avec succes.', type: 'success' });

                            } else {
                                this.$dispatch('toast', { message: data.message || 'Une erreur est survenue', type: 'error' });
                            }

                        } catch (e) {
                            console.error(e);
                            this.$dispatch('toast', { message: 'Erreur reseau', type: 'error' });
                        } finally {
                    ...
                    resetForm() {
                        ...
                        this.buildTs && this.buildTs.clear(true);
                    }
                }
            }
        </script>
            ...
            <div class="p-6">
                ...
                <div class="mb-6 space-y-1.5 text-center sm:text-left border-b border-gray-200 pb-4">
                    ...
                    <p class="text-sm text-muted-foreground">
                        Verifiez les composants et le paiement avant validation.
                    </p>
                </div>
                ...
                <div class="grid gap-4 py-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-500">Total pieces</span>
                        <span class="font-semibold" x-text="cart.reduce((sum, item) => sum + parseInt(item.quantity), 0)"></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-500">Sous-total</span>
                        <span class="font-semibold" x-text="formatCurrency(subtotal)"></span>
                    </div>
                    <div class="flex items-center justify-between" x-show="laborSubtotal > 0">
                        <span class="text-sm font-medium text-gray-500">Main d'oeuvre</span>
                        <span class="font-semibold" x-text="formatCurrency(laborSubtotal)"></span>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-red-600" x-show="globalDiscount > 0">
                        <span class="text-sm font-medium">Remise supplementaire</span>
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 pt-2 mt-2">
                        <span class="text-lg font-bold">Total facture</span>
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 pt-2 mt-2" x-show="payment.method === 'cash'">
                        <span class="text-sm font-medium text-gray-500">Montant recu</span>
                    ...
                    <div class="flex items-center justify-between" x-show="payment.method === 'cash'">
                        <span class="text-sm font-medium text-gray-500">Monnaie</span>
                ...
                <div class="mt-6 border-t border-gray-200 pt-4 space-y-4">
                    <!-- Status Selection -->
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Statut de vente</label>
                        <div class="grid grid-cols-2 gap-2">
                            <button
                    ...
                    >
                        ...
                        <span x-text="isSubmitting ? 'Traitement...' : 'VALIDER LE DOSSIER'"></span>
                    </button>
        ...
        <!-- Create Client Modal (Still using Blade/Alpine Hybrid for ease if reusable) -->
        <x-modal name="customer-modal" focusable>
            <div class="p-6" x-data="{
                    ...
                    try {
                        ...
                        if (res.ok) {
                            ...
                            if (data.errors) {
                                ...
                                this.$dispatch('toast', { message: data.message || 'Erreur lors de la creation du client', type: 'error' });
                            }
                        }
                ...
                <div class="mb-6 space-y-1.5 text-center sm:text-left border-b border-gray-200 pb-4">
                    <h3 class="text-lg font-semibold leading-none tracking-tight text-foreground">
                        {{ __('Creer un nouveau client') }}
                    </h3>
                    <p class="text-sm text-muted-foreground">
                        {{ __('Ajoutez un nouveau client pour ce dossier ambulance.') }}
                    </p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <!-- Address -->
                    <div class="space-y-2">
                        <x-input-label for="new_address" :value="__('Adresse')" />
                        <textarea
                            id="new_address"
                    ...
                    <div class="mt-6 flex justify-end gap-3 border-t border-gray-200 pt-4">
                        ...
                        <x-primary-button type="button" @click="save()" x-bind:disabled="loading">
                            ...
                            <span x-text="loading ? 'Enregistrement...' : 'Enregistrer le client'"></span>
                        </x-primary-button>
                    </div>
                </div>
            </div>
        ...
        <x-modal name="cancel-modal" focusable>
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Voulez-vous vraiment annuler ? Les pieces, le client et les informations saisies seront effaces.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                <x-danger-button @click="resetForm(); clearStorage(); $dispatch('close-modal', { name: 'cancel-modal' }); $dispatch('toast', { message: 'Dossier annule', type: 'info' })" class="w-full sm:w-auto justify-center">
                    {{ __('Oui, annuler le dossier') }}
                </x-danger-button>
                <button
                    type="button"
        ...
        <div @customer-created.window="selectedClient = $event.detail; customerSearch = '';"></div>
    </div>
</x-app-layout>
